...
if [[ -z "$resolveip" && -z "$resolvedns" ]]; then
    PS3='请选择您的网络模式:1：ip 2：域名 (请输入1后回车):'
    WAN=("IP" "DNS/Domain")
...
EOF
    ...
    echo -e "----安装完毕,by 九州科技@xiaowu6688-----------------"
    echo -e "服务器ip地址为： ${wanip}"
    echo -e "服务器密钥为： ${key}"
    echo -e "请选择是否需要网页版支持: 1:需要 2：不需要 (请输入2回车)"
...
if [[ -z "$http" ]]; then
    PS3='是否需要网页支持：1：需要 2：不需要 （请输入2回车）:'
    EXTRA=("Yes" "No")
